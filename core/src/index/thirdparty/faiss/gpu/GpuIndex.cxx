...
GpuIndex::searchFromCpuPaged_(int n,
  ...
  while (cur3 < n) {
    ...
    if (cur2 != -1 && cur2 < n) {
      ...
      HIP_VERIFY(hipMemcpyAsync(bufGpus[cur2BufIndex]->data(),
      ...
      eventPinnedCopyDone[cur2BufIndex] =
        std::move(std::unique_ptr<HipEvent>(new HipEvent(copyStream)));
    ...
    if (cur3 != -1 && cur3 < n) {
      ...
      eventGpuExecuteDone[cur3BufIndex] =
        std::move(std::unique_ptr<HipEvent>(new HipEvent(defaultStream)));
