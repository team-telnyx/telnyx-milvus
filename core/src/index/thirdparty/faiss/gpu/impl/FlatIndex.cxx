...
void
FlatIndex::reserve(size_t numVecs, hipStream_t stream) {
// TODO: HADI
// #ifdef FAISS_USE_FLOAT16
//   if (useFloat16_) {
...
FlatIndex::getVectorsFloat32Copy(hipStream_t stream) {
  return getVectorsFloat32Copy(0, num_, stream);
}

DeviceTensor<float, 2, true>
FlatIndex::getVectorsFloat32Copy(int from, int num, hipStream_t stream) {
...
void
FlatIndex::add(const float* data, int numVecs, hipStream_t stream) {
  if (numVecs == 0) {
    return;
  }
