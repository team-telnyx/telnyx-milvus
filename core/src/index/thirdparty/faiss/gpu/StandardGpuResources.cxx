...
void
StandardGpuResources::setDefaultStream(int device, hipStream_t stream) {
  auto it = defaultStreams_.find(device);
  if (it != defaultStreams_.end()) {
    // Replace this stream with the user stream
    HIP_VERIFY(hipStreamDestroy(it->second));
    it->second = stream;
  }
...
void
StandardGpuResources::setHipMallocWarning(bool b) {
  hipMallocWarning_ = b;

  for (auto& v : memory_) {
    v.second->setHipMallocWarning(b);
  }
}
...
StandardGpuResources::initializeForDevice(int device) {
  ...
  if (it != userDefaultStreams_.end()) {
    ...
    HIP_VERIFY(hipStreamCreateWithFlags(&defaultStream,
  ...
  HIP_VERIFY(hipStreamCreateWithFlags(&asyncCopyStream,
                                        hipStreamNonBlocking));
  ...
  for (int j = 0; j < kNumStreams; ++j) {
    ...
    HIP_VERIFY(hipStreamCreateWithFlags(&stream,
                                          hipStreamNonBlocking));
...
// #endif
  ...
  auto mem = std::unique_ptr<StackDeviceMemory>(
    new StackDeviceMemory(device,
                          // adjust for this specific device
                          getDefaultTempMemForGPU(device, tempMemSize_)));
  mem->setHipMallocWarning(hipMallocWarning_);
